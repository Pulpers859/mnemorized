...
def _gemini_prompt_director_system_prompt() -> str:
    ...
    return (
        ...
        "anchor legibility and coherent spatial layout while keeping the image free of "
        "numeric/dose/formula text. "
        ...
        "- Use silhouette-first anchor language. Any visible text is a short sound-alike NAME only, and secondary.\n"
        "- NO PRECISION TEXT: never ask the image to render numbers, digits, doses, units, thresholds, lab values, or formulas as text — those exact facts live in the narration and flashcards, not the picture. A number may appear only when encoded as a visual hook (a shape look-alike or a small countable quantity), never as spelled-out digits on a device.\n"
        "- Avoid descriptive-noun label leakage: never ask for words like 'pharmacy label' or 'warning sign' to render.\n"
        "- Avoid adjacent duplicate characters on glowing or digital screen text; use physical objects instead.\n"
        "- Keep visible name labels sparse and large enough to read; render zero numeric/formula text.\n\n"
        "JSON SCHEMA:\n"
        "{"
